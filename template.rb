...
# Set database collation and password
run "ruby -pi.bak -e 'gsub(/password:/, \"password: root\")' config/database.yml"
run "ruby -pi.bak -e 'gsub(/adapter: mysql2/, \"adapter: mysql2\n  host: database\n  collation: utf8mb4_unicode_ci\")' config/database.yml"
run "rm ./config/database.yml.bak"
...
if File.exist?("app/assets/stylesheets/application.css") # don't need  design files for api only app
  # Add design files
  ...
  # append importing spacing to application.scss
  append_file 'app/assets/stylesheets/application.scss', "@import 'custom/spacing';\n"
...
DT = ["dotenv-rails", "rspec-rails", "factory_bot_rails", "rubycritic", "ffaker", "pry-rails"]
DEV = ["rubocop", "rubocop-performance", "rubocop-rails", "rubocop-rspec", "annotate"]

def install_gems

  DT.each do |rgem|
    run "bundle add #{rgem} --group 'development, test' --skip-install"
  end

  DEV.each do |rgem|
    run "bundle add #{rgem} --group 'development' --skip-install"
  end
...
HEREDOC
  ...
  if File.exists?("config/webpacker.yml")
...
inject_into_file 'app/controllers/application_controller.rb', after: %r{class ApplicationController < ActionController} do
  ...
  def http_basic_authenticate
    ...
    authenticate_or_request_with_http_basic Rails.env do |name, password|
      name == '#{@app_name}' && password == 'password'
    end
  end
  RUBY
end
